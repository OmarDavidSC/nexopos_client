...
export class ClientesComponent extends FormularioBase implements OnInit {

  ListaClientes: ECliente[] = [];
  UsuarioActual: Eusuario | null = null;
  Role: ERol | null = null;

  displayedColumns: string[] = ['TipoDocumento', 'NumeroDocumento', 'Nombre', 'Telefono', 'Correo', 'Direccion', 'Acciones'];
  ...
  constructor(
    ...
    super('clientes', dialog, route, router, spinner)
  }
  ...
  async eventoEliminar(item: ECliente): Promise<void> {
    const confirmToast = this.toastService.show(
      '¿Confirmas la eliminación del cliente?', 'Eliminar cliente',
    ...
    confirmToast.onAction.subscribe(async () => {
      ...
      try {
        const { success, data, message } = await this.clienteService.remove(item.Id);
        this.toastService.clear();
        if (success) {
